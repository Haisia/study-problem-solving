...
- 티셔츠는 S, M, L, XL, XXL, XXXL의 6가지 사이즈가 있습니다. 티셔츠는 같은 사이즈의 \(T\)장 묶음으로만 주문할 수 있습니다.
...
\((0 \leq S, M, L, XL, XXL, XXXL \leq N; S + M + L + XL + XXL + XXXL = N)\)

셋째 줄에 티셔츠와 펜의 묶음 수를 의미하는 정수 \(T\)와 \(P\)가 공백으로 구분되어 주어집니다.  
\((2 \leq T, P \leq 10^9)\)
...
다음 줄에 펜을 \(P\)자루씩 최대 몇 묶음 주문할 수 있는지와, 그때 펜을 한 자루씩 몇 개 주문하는지 출력하세요.
